...
const DropUser = (myProp: Props) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            const url = `/employee/${myProp.item.id}`;
            ...
            if (response.data.success) {
                toast.success(message, { containerId: `toastDrop-${myProp.item.id}` });
                setShow(false);
                setTimeout(() => router.refresh(), 1000);
            } else {
                toast.warning(message, { containerId: `toastDrop-${myProp.item.id}` });
            }
        } catch (error) {
            console.error(error);
            toast.error("Something went wrong", { containerId: `toastDrop-${myProp.item.id}` });
        }
    };
            ...
            <Modal isShow={show}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="w-full p-3 rounded-t-lg">
                        <h1 className="font-semibold text-lg">Hapus Pengguna</h1>
                        <span className="text-sm text-slate-500">
                            Apakah Anda yakin ingin menghapus Admin {myProp.item.name}?
                        </span>
                    </div>
